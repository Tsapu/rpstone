...
#include "ta_robot2_viz/temoto_action.h"
#include "temoto_visualization_manager/visualization_manager_interface.h"

/* 
 * ACTION IMPLEMENTATION of TaRobot2Viz 
 */
class TaRobot2Viz : public TemotoAction
{
public:
...
void executeTemotoAction()
{
  ...
  TEMOTO_INFO_STREAM("Showing robot '" << in_param_robot2_name << "' in RViz ...");
  std::string robot_description_rviz_config = "{Robot Description: " + in_param_robot2_description + "}";
  vmi_.loadRvizPlugin("rviz/RobotModel", "", robot_description_rviz_config, in_param_robot2_name + " model");
  TEMOTO_INFO_STREAM("Robot '" << in_param_robot2_name << "' visualized");
...
{
  in_param_robot2_description = GET_PARAMETER("robot2_description", std::string);
...
std::string in_param_robot2_description;
...
CLASS_LOADER_REGISTER_CLASS(TaRobot2Viz, ActionBase);
